...
import { FiDollarSign, FiLock, FiMonitor, FiSmile, FiTrendingUp } from 'react-icons/fi';
...
const NavItem = ({ children, href, icon, path }: NavItemProps) => {
  ...
  return (
    <Link
      className={cn(
        'flex items-center gap-3 rounded-lg px-3 py-2 text-foreground hover:text-black hover:dark:text-gray-500',
        isActive && 'bg-muted dark:bg-secondary text-foreground'
      )}
      href={href}
    >
      {icon}
